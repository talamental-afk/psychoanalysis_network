...
# -*- coding: utf-8 -*-
"""
将循环论证详细说明集成到psychoanalysis_data.ts中
"""
...
def integrate_circular_details(data_file):
    """将循环论证详细说明集成到数据文件中"""
    with open(data_file, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # 对每个循环论证概念进行处理
    for concept_id, details in CIRCULAR_DETAILS.items():
        # 查找该概念的定义
        pattern = rf"(id:\s*'{re.escape(concept_id)}'.*?hasCircularLogic:\s*true.*?circularLogicExplanation:\s*'[^']*')"
        
        def replace_func(match):
            concept_block = match.group(1)
            
            # 添加详细说明
            argument_process = details['argumentProcess'].replace("'", "\\'")
            logical_problem = details['logicalProblem'].replace("'", "\\'")
            circular_chain = ', '.join([f"'{item}'" for item in details['circularChain']])
            visualization = details['visualization'].replace("'", "\\'")
            academic_significance = details['academicSignificance'].replace("'", "\\'")
            
            detailed_explanation = f"""{concept_block},
    argumentProcess: '{argument_process}',
    logicalProblem: '{logical_problem}',
    circularChain: [{circular_chain}],
    visualization: '{visualization}',
    academicSignificance: '{academic_significance}'"""
    ...
    print(f"已为 {len(CIRCULAR_DETAILS)} 个循环论证概念添加详细说明")

if __name__ == '__main__':
    integrate_circular_details('/home/ubuntu/psychoanalysis_network/psychoanalysis_data.ts')
    print("循环论证详细说明集成完成！")
